...
import { useNavigate } from 'react-router-dom'; // Para redirigir a la página de registrar persona


const HeaderDisabilitys = ({ toggleSidebar, onLogout }) => {
  const [searchOpen, setSearchOpen] = useState(false);
  const [showLogout, setShowLogout] = useState(false); // Estado para mostrar el menú de cerrar sesión
  const showSearchFull = useBreakpointValue({ base: false, lg: true });
  const navigate = useNavigate(); // Hook para navegar a la página de registro

  const handleAddPerson = () => {
    navigate('/registrar-persona'); // Redirigir a la vista del formulario de registro
  };
  ...
  const handleLogout = () => {
    ...
    onLogout(); // Llama a la función de logout para actualizar el estado global
  };
...
export default HeaderDisabilitys;
